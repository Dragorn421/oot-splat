...
def find_compiler_command_line(
    ...
    for line in make_log:
        parts = line.split()
        if "./tools/asm-processor/build.py" in parts and "-o" in parts and str(filename) in parts:
...
def run_cfe(
    ...
    # python3 ./tools/asm-processor/build.py ... [INPUT_FILE]
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        try:
            ...
            subprocess.run(rest[:2] + ["--keep-preprocessed", tmpdir] + rest[2:] + ["-Hf", input_file], check=True)

            preprocessed_c_p, = Path(tmpdir).iterdir()

            stem = preprocessed_c_p.stem.removeprefix(input_file.stem + "_")
            ...
            if not keep_files:
                if symbol_table_file is not None:
                    symbol_table_file.unlink(missing_ok=True)
                if ucode_file is not None:
